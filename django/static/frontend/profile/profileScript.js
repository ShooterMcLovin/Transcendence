...
function fetchFriends(friends) {
    ...
    friends.forEach(friend => {
        ...
        removeButton.dataset.username = friend.username;
...
async function selectAvatar(imageSrc) {
    try {
        ...
        const response = await fetch('/api/avatar_url', {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${sessionStorage.getItem('token')}`, // Si nécessaire
            },
            body: JSON.stringify({ avatar_url: imageSrc }),
        });
        ...
        hideMenu(); // Fermer le menu après la sélection
    }
}
...
//     const userAvatar = document.getElementById("userAvatar");
//     userAvatar.src = imageSrc;
//     hideMenu(); // Ferme le menu après la sélection
// }

// export{showMenu,hideMenu,selectAvatar}